...
{{"rat0"},{"options","Continue"}},
{{"rat1"},{"options","No","Yes"}},
};

String[][][] judgment =
{
{{"judgment"}, {"nelem","2"},{"musthave","0"},{"group","0"},{"gminmax","2.2"},{"gplace","0"},{"gorder","0"},{"gsequence","fixed"}},
{{"judgment1"},{"choices","rat1"},{"stem","A corporate personnel officer is evaluating a Black job applicant and a White job applicant who are identically qualified except the White applicant has more prior experience in related work. Is there a reasonable justification for this personnel officer hiring the Black applicant rather than the White applicant?"}},
{{"judgment2"},{"choices","rat1"},{"stem","A college admissions officer considers applications from Black applicants and White applicants with similar credentials and cannot accept all of them. Should the admissions officer more often accept Black applicants than White applicants?"}},
};  
...
//   if ((curitem==0)) preclass="medblue2";
 ...
 // var minselect = js_minselect[curitem];
  ...
  var prefix2 = '<button type="button" style="font-size:14; font-weight: bold;text-align:left; width: 100px; background-color:';
  
  
  var prefixskip = '<button type="button" style="font-size:10; font-weight: bold; width: 100 px; background-color:';
 var curdisplay='<table width="70%" align=center><tr><td>'+'<font color="#AAAAAA"><span class=small><b>Survey: '+(curitem+1)+'/'+js_nitem+'</b></span></font><br /> <span  class='+preclass+'>'+js_prefix[curitem]+'</span><br/><span class=big style="background-color: #FFFFCC">'+js_stem[curitem]+'</span><br/><br/></td></tr></table> <table width="70%" align=center cellpadding="2">';
   ...
   if (js_varnames[curitem]=='vote.vote06') 
   {
   
    curdisplay+='<tr><td class=med>';
    
   curdisplay+='<align=center><input type=text style="font-size:16;"  maxlength=5 size=5 id="texta1" name="texta1" onkeypress="keyCode=(event.which)?event.which:event.keyCode;return (keyCode != 13); "onkeydown="keyCode=(event.which)?event.which:event.keyCode;return (keyCode != 13); "> <br/><br/>';
   
   curdisplay+='<button type="button" id="thisbut" style="font-size:16; font-weight: bold; width: 100px; background-color:#EEEEEE" onmouseup=\'changestate("texta1",5,"thisbut")\'>Next</button></td></tr>';     
    }
    else 
  ...
  // the finished button
   ...
   if (maxselect>1) {
          ...
          curdisplay+='>Click here</button></td></tr>';
         
    curdisplay+='<tr><td  class="sml" colspan="'+icols+'"><br>Select an item by clicking on it and turning it <u>BLUE</u>.<br/>To unselect, click it once more.<br>When you are done selecting the required number of items you can click on the &ldquo;Click here&rdquo; button.<br/>The Finished button will turn <u>YELLOW</u> and you can confirm by clicking it a second time.<br/><br/></td></tr>';
}
   
  
   if ((curitem<2) && (maxselect<=1)) curdisplay+='<tr><td colspan="'+icols+'">Selecting an answer once colors it yellow.<br/> You can change your answer by selecting another option. <br /> To confirm, click the selected (yellow) button a second time.<br/><br/></td></tr>';
 ...
 } else alert('Please type in your zip code or select decline to answer');
